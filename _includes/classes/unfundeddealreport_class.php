...
class unfundeddealreport {
	...
	public function execute(reportwherebuilder &$wherebuilder, reportfrombuilder &$frombuilder, reportorderbuilder &$orderbuilder){
		...
		$sql = 'SELECT
				...
				CONCAT("$",FORMAT(d.discount,2)) discount
				' . $fromclause . '
		...
		$result['data'] = db_select_assoc_array($sql);		
		
		//$salesresult = db_select_assoc_array($salessql);
		$totalresult = db_select_assoc($totalsql);
		$result['info'] = $totalresult;
		
		$totaldata = array('amount_financed'=>$totalresult['financed_total'], 'discount'=>$totalresult['discount_total'], 'lender'=>'TOTAL', 'stock'=>$totalresult['deals']);
		$avgdata = array('amount_financed'=>$totalresult['financed_average'], 'discount'=>$totalresult['discount_average'], 'lender'=>'Average');
		...
		*/
		...
		if($result){
			...
			$sprint_lender = '<a href="#" onclick="poplender(%d);">%s</a>';
			...
			foreach($result as $row){
				...
				$result[$count]['lender'] = sprintf($sprint_lender, $row['lender_id'], $row['lender']);
				$count++;
			}
			*/
		...
		$table->add_get_var('action', 'unfunded');
