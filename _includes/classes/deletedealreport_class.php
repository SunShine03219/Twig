...
class deletedealreport {
	...
	public function execute(reportwherebuilder &$wherebuilder, reportfrombuilder &$frombuilder, reportorderbuilder &$orderbuilder){
		...
		if($result){
			//play with data
			$count = 0;
			$sprint_edit = "<a href=\"deals.php?id=%d&amp;action=delete\">%s</a>";
			...
			foreach($result as $row){
				...
				$result[$count]['closed_dms'] = ($row['closed_dms']==0?'Open':'Closed');
				$count++;
			}
			*/
			foreach($result as $row){
		...
		$cl->add('closed_dms', 'DMS Status', '8%', 'closed_dms', 'tcen');
		...
		$cl->add('vehicle', 'Vehicle', '14%', 'vehicle', 'ttxt');
		...
		$table->add_get_var('action', 'delete');
