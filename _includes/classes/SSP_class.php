	...
	static function filter ( $request, $columns, &$bindings )
	{
		...
		if ( isset( $request['columns'] ) ) {
			for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
				...
				$str = strtolower($requestColumn['search']['value']);
...
//PrintVar($bindings);exit;
	...
	/**
	 * The difference between this method and the `simple and complex` one, is that you can
	 * apply additional `where` and `join` and `group by` conditions to the SQL queries. These can be in
	 * one of two forms:
	 *
	 * * 'Result condition' - This is applied to the result set, but not the
	 *   overall paging information query - i.e. it will not effect the number
	 ...
	 *  @param  string $primaryKey Primary key of the table
	 *  @param  array $columns Column information array
	 *  @param  string $query Column information array
	 *  @param  string|array $where WHERE condition to apply to all queries
	 *  @param  string $join condition to apply to all queries
	 *  @param  string $groupBy condition to apply to all queries
	 *  @param  string $orderBy condition to apply to all queries
	 *  @return array Server-side processing response array
	 */
	static function joinGroupComplex (
		$request,
		// $conn,
		$table,
		$primaryKey,
		...
		// Build the SQL query string from the request
		$limit = self::limit( $request, $columns );
		$sql = $select.' FROM '.$table.$join.$where.$groupBy.$orderBy.' '.$limit;
		...
		foreach($data as $key => $row){
			$temp = array();
			$temp[0] = $row['customer'].' - '.$row['stock'];
			$temp[1] = $row['sales_team'];
			$temp[2] = $row['status'];
			$temp[3] = $row['date_due'];
			$temp[4] = $row['amount'];
			$temp[5] = $row['method'];
		...
		$sql = $select.' FROM '.$table.$join.$where.$groupBy;
		...
		// Return all
		$totalRecords = $stmt->fetchAll( PDO::FETCH_BOTH );
