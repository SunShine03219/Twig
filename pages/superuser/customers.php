<?php
    ...
    if (isset($_GET['id']) || isset($_POST['id'])) {
        $isID = isset($_GET['id']) ? $_GET['id'] : $_POST['id'];
        $stripeCheck = isset($_POST['action']) ? $_POST['action'] : null;
        $twig->addGlobal('page_title', 'Edit Customer');
        $twig->addGlobal('session', $_SESSION['FT']);
        $twig->addGlobal('id_costumer', $isID);
        $stripeCustomerId = null;

        if($stripeCheck !== null){
            $company = new company();
            $company->make_me_into($isID);
            $checkCustomer = $company->get_stripe_customer_id();
            if(!$checkCustomer){
                \Stripe\Stripe::setApiKey(STRIPE_SECRET);
                $customer = \Stripe\Customer::create(array(
                    "name" => $company->get_name()
                ));
                $company->set_stripe_customer_id($customer->id);
                $out = $company->update_one();
                $stripeCustomerId = $company->get_stripe_customer_id();
                $_SESSION['FT']['stripe_customer_id'] = $stripeCustomerId;
            }
        ...
        if (isset($_GET['id'])) {
            ...
            if($company->make_me_into($id)){
                ...
                $twig->addGlobal('stripe_customer_id', empty($stripeCustomerId) ? $company->get_stripe_customer_id() : $stripeCustomerId);
                $nonce = new nonce();
        ...
        $twig->addGlobal('error', isset($_SESSION['error']) ? $_SESSION['error'] : null);
         ...
         * Render Templates
         */
        echo $twig->render('superuser/tpl.customers.twig');
    }