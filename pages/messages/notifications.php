...
if($_SESSION['FT']['SEC_SUPPORT']){
    if (isset($_GET['id']) || isset($_POST['id'])) {
        ...
        $twig->addGlobal('page_title', 'Notifications Center');
        $twig->addGlobal('session', $_SESSION['FT']);
        if(isset($_GET['status'])) {
            $twig->addGlobal('status', $_GET['status']);
        }
        $controller = new messages();
        $output = $controller->invoke(null);
    ...
    $controller = new messages();
    $output = $controller->invoke("user");
    $twig->addGlobal('messages', $output);

    /*
        * Render Templates
        */
    echo $twig->render('messages/tpl.notifications_user.twig');
}
