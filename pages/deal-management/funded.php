...
if(isset($_GET['dropdown'])) {
    ...
    echo $controller->getFundedTableList($_GET, $_SESSION['FT']['company_id']);
} else {
    $twig->addGlobal('page_title', 'Funded Deals');
    $controller = new dealcontroller();
    //$output = $controller->invoke('funded');

    $company_id = $_SESSION['FT']['company_id'];
    $str_where = "company_id = $company_id AND funded = 1 AND closed_dms = 0 ";
    ...
    $_SESSION["current_page"] = "funded";
    $_SESSION['FT']['current_page'] = 'funded';
    ...
    echo $twig->render('deal-management/tpl.funded.twig');
}
